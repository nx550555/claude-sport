...
### CE016 — upset_patterns 勝敗逆転記録（PA051監査発覚）
**日付:** 2026-04-20（Session_46 発覚）
**カテゴリ:** DATA_ERROR（CE013/CE014 と同種 — 勝敗真逆記録）
**ステータス:** MITIGATED

**何を間違えたか:**
PA051 監査（upset_patterns 28件 ③→④反映）の過程で、factors 空の4件（A017/A022/A023/A024）を個別 WebSearch で深掘りしたところ、以下2件で勝敗が真逆に記録されていたことが判明：

1. **A017** Fonseca vs Shelton Munich QF (2026-04-17)
   - 記録: "Fonseca d. Shelton" (UPSETとして登録、fav Shelton @1.44)
   - 実際: **Shelton d. Fonseca 6-3 3-6 6-3**（ATP公式+Yardbarker+Tennis.com複数確認）
   - Shelton No.2 seed・市場favorite、1stサーブ 84% で勝利 — UPSETではなく本命HIT

2. **A023** Gea vs Gaubas Madrid Q R1 (2026-04-20)
   - 記録: "Gea d. Gaubas" (UPSETとして登録、fav Gaubas @1.72)
   - 実際: **Gaubas d. Gea 6-4 6-2**（scores24.live + LiveScore確認）
   - Gaubas 22nd seed・市場favorite — UPSETではなく本命HIT

**追加2件も再検証完了・全4件逆転確定（2026-04-20 同日中に解消）:**
- **A022** Svajda vs Sakamoto: **Sakamoto d. Svajda 7-6 6-1** (tennismajors + Wikipedia)。market fav Sakamoto @1.59 HIT. 6 aces/1 DF dominant serve.
- **A024** Virtanen vs Drogue(t): **Droguet d. Virtanen (3セット)** (Wikipedia qualifying draw)。market fav Droguet @1.72 HIT. Virtanen 1st set 6-2 lead後 Droguet 2セット回収. clay specialist endurance が Virtanen fast-start を上回る.

**最終結論: 4件全て同一パターンの勝敗逆転記録**。CE013/CE014/CE015 の延長として深刻。Madrid Q R1 (2026-04-20) で3件連続発生 → **手動データ取込時の勝敗欄記入プロセスに構造的不具合あり**。

**どう発覚したか:**
PA051 監査で factor_notes 空エントリに factor 分析を追加するため WebSearch 実施 → 記録された勝者と検索結果の勝者が食い違うことで発見。

**なぜ発生したか:**
- Madrid Q (4/20) と Munich QF (4/17) いずれも手動データ取込時の勝敗欄記入ミス
- 「fav odds が低い側が勝った = 本命HIT」と「underdog odds が高い側が勝った = UPSET」の機械的変換で、勝者と敗者のラベルを取り違えた可能性
- CE013(UFL Renegades), CE014(GB001 Ostapenko), CE015(MC SF相手名)と同系列の**勝敗真逆パターン**

**修正内容:**
- upset_patterns.json: A017/A023 を `invalidated_upsets` 配列へ移動。`invalidated: true` + `ce_error: CE016` + `actual_result_corrected` + `verification_sources` 記録
- A022/A024 は `verification_status: NEEDS_RE_VERIFICATION` タグ付与
- confirmed_upsets: 36件 → 34件

**再発防止ルール（強化版）:**
→ upset_patterns に新規エントリを登録する際、**winner名と fav_odds/market_favorite の整合性を必ず検証する**
  - "result: X d. Y" と "market_favorite: Y" の組み合わせ = UPSET 成立
  - "result: X d. Y" と "market_favorite: X" の組み合わせ = **これは UPSET ではない**（記録すべきでない）
→ factor_notes が空のまま confirmed_upsets に残すのは禁止（4件が空のまま約数日残存していた事実が問題）
→ PA051 監査のような定期スキャンで自動検知するため health_check.py v3 に「confirmed_upsets の winner vs market_favorite 整合性チェック」を追加検討

**教訓:**
- 「タグ付けだけ」の作業と思っていた PA051 監査が、実は**過去の data_error を発見する監査**としても機能した
- MISS分析の rule_linked 補填作業は、同時に**記録品質監査の機能**を持つことが判明
- factor_notes 空エントリは「分析未完了」だけでなく「そもそも誤記録の可能性」を警戒すべき

---

### CE017 — Madrid Q R1 records 勝敗逆転記録 4件（CE016 の records 側未訂正延長）
**日付:** 2026-04-23 発覚（元データ入力: 2026-04-20）
**カテゴリ:** DATA_ERROR（CE013/CE014/CE016 と同種・勝敗真逆記録の連続発生）
**ステータス:** MITIGATED

**何を間違えたか:**
records/tennis/2026-ATP.json の Madrid Q R1 (2026-04-20) 試合 4件で勝敗が逆転記録されていた:

1. **#73 Svajda vs Sakamoto**: records=Svajda 勝利 / 実際は **Sakamoto d. Svajda 7-6(9) 6-1**
2. **#75 Gea vs Gaubas**: records=Gea 勝利 / 実際は **Gaubas d. Gea 6-2 6-4**
3. **#77 Virtanen vs Droguet**: records=Virtanen 勝利 / 実際は **Droguet d. Virtanen 2-6 6-3 6-3**
4. **#94 Travaglia vs Basilashvili**: records=Travaglia 勝利 / 実際は **Basilashvili d. Travaglia 7-6(4) 6-4**

4件すべて市場 favorite が予測通り勝利。outcome_note には "UPSET" と書かれていたが、実態は fav HIT。

**どう発覚したか:**
PA071 (MISS miss_analysis 欠損 58件補填) 作業中、Session_46 で訂正された upset_patterns A022/A023/A024 と records 側の result が矛盾することに気付き、WebSearch で再検証。#94 は upset_patterns に未登録だったが outcome_note "favorite win" と prediction_hit=False の論理矛盾から発見。

**なぜ発生したか:**
- Session_46 の CE016 修正は upset_patterns.json のみで完結し、**同じデータの記録元である records 側は同期されなかった**（修正範囲の漏れ）
- Madrid Q R1 取込時の機械的変換ミス: 「fav が低 odds 側 + UPSET オッズに振った」という仮定で勝敗が反転した可能性
- outcome_note の "UPSET" 表記と一次ソース照合を怠った
- result/score "確認中" や上書き作業時の verification 欠如

**修正内容:**
- 4件すべて result/score/prediction_hit/outcome_note を訂正、ce_correction フィールド追記、verification_sources 4件分明示
- prediction_hit: False → True (4件) → cumulative.json/dashboard_stats.json で ATP の予測精度数値が変化
- #73/#75/#77 は upset_patterns A022/A023/A024 (invalidated_upsets) と整合確認
- #94 は UPSET 不成立のため upset_patterns 新規登録不要

**再発防止ルール（強化版）:**
→ **CE013/CE014/CE015/CE016/CE017 は全て同根**: 勝敗逆転記録パターンを根本的に防ぐ仕組みが必要
→ **データ訂正時は records と upset_patterns を必ずペアで更新**（片方のみ訂正は禁止）
→ **outcome_note "UPSET" / "favorite win" 表記と prediction_hit / market_favorite フィールドの論理整合性を health_check で自動検証**（Session_51 で次セッション以降に追加検討）
→ **手動データ取込時の機械的変換に頼らず、各試合 result の一次ソース確認を独立に実施**（snippet 依存禁止）
→ **outcome_note の "UPSET" 表記と一次ソースの再照合を miss_analysis 補填時の必須プロセスに組み込む**

**教訓:**
- PA071 (MISS 補填) のような monitoring task が、品質監査として隠れた data_error を継続検知する役割を持つ
- CE016 修正時に "upset_patterns のみ訂正" でクローズせず、records/dashboard_stats/cumulative の波及確認まで行うべきだった
- 外部データ取込時の勝敗欄記入は最弱点 → 独立検証ステップを必ず挟む
