...
RECORDS = {
    ...
    "premiership": BASE / "records/premiership/2026.json",
    "top14":       BASE / "records/top14/2026.json",
    "prod2":       BASE / "records/prod2/2026.json",
    "ahl":         BASE / "records/ahl/2025-26.json",
}
...
# ─────────────────────────────────────
SPORT_CARDS = [
    {"key": "ATP", "data_key": "atp",         "label1": "GO回数",  "color": "#79c0ff", "tier": "advanced"},
    {"key": "WTA", "data_key": "wta",         "label1": "GB推奨",  "color": "#f9a8d4", "tier": "advanced"},
    {"key": "NHL", "data_key": "nhl",         "label1": "GO回数",  "color": "#a5f3fc", "tier": "advanced"},
    {"key": "UFL", "data_key": "ufl",         "label1": "GO回数",  "color": "#fca5a5", "tier": "basic", "extra": "UFL"},
    {"key": "NRL", "data_key": "nrl",         "label1": "GO回数",  "color": "#86efac", "tier": "basic"},
    {"key": "SRP", "data_key": "superrugby",  "label1": "GO回数",  "color": "#c4b5fd", "tier": "basic"},
    {"key": "NBA", "data_key": "nba",         "label1": "GO回数",  "color": "#fca5a5", "tier": "advanced", "extra": "NBA"},
    {"key": "SL",  "data_key": "superleague", "label1": "GO回数",  "color": "#bfdbfe", "tier": "basic"},
    {"key": "PREM", "data_key": "premiership", "label1": "GO回数", "color": "#fde047", "tier": "basic"},
    {"key": "TOP14","data_key": "top14",       "label1": "GO回数", "color": "#fca5a5", "tier": "basic", "extra": "Top 14"},
    {"key": "PROD2","data_key": "prod2",       "label1": "GO回数", "color": "#93c5fd", "tier": "basic", "extra": "Pro D2"},
    {"key": "AHL",  "data_key": "ahl",         "label1": "GO回数", "color": "#a5f3fc", "tier": "basic", "extra": "AHL"},
]
...
def build_sport_card(card, metrics_html, content):
    ...
    # Inject tier badge if not already present
    tier = card.get("tier")
    if tier and 'tier-badge' not in header_html:
        badge_label = '🅰 Advanced' if tier == 'advanced' else '🅱 Basic'
        badge_cls = 'tier-adv' if tier == 'advanced' else 'tier-basic'
        badge_html = f' <span class="tier-badge {badge_cls}">{badge_label}</span>'
        # Insert before <span class="rule-ver">
        rv_idx = header_html.find('<span class="rule-ver">')
        if rv_idx > 0:
            header_html = header_html[:rv_idx] + badge_html + ' ' + header_html[rv_idx:]
...
def update_overview(content, overview):
    # Tier-based overview is regenerated by _tmp_dashboard_tier.py / regenerate_overview_tier below
    # Legacy big-stat pattern matching is disabled because the overview structure has changed.
    regenerate_overview_tier(content)
    return content

def regenerate_overview_tier(content):
    '''Regenerate tier-separated big-stats from dashboard_stats.json. Currently a placeholder;
    for now, the overview is preserved as-is (manual edit via _tmp_dashboard_tier.py).
    TODO: Implement dynamic tier-stat regen reading core/dashboard_stats.json.by_tier'''
    pass

def _legacy_update_overview_disabled(content, overview):
