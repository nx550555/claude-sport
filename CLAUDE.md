...
8. **(Session_64 2026-04-29 議題2 追加) 結果反映の処理パターン分類**: A-3 サブタスク3 で 9件の結果反映を実施した際、3つの処理パターンが顕在化した。本ステップは各パターンの記録運用を明文化し、Claude の判断揺れと health_check 検査対象漏れを構造的に防止する。

   - **パターンA: 既存遡及更新 (games[] エントリあり / 結果未反映)**:
     - **判定条件**: 既存 records/{sport}/*.json の games[] / predictions[] 配列内に該当エントリが存在し、`status="pending"` / `result=null` / `prediction_hit=null` のいずれかで結果が未反映の場合
     - **処理ルール 6項目**:
       ...
       4. **`prediction_hit_updated_at` フィールドを `step05_scanned_at` と同一 ISO 8601 UTC タイムスタンプで同時付与** (議題2 案γ 採用 / 検査対象捕捉のため必須)
       5. `miss_analysis` (5種タグ運用) / `outcome_note` / `verification_sources` 等の詳細記録フィールドを追加
       6. 必要に応じて `result_reflection_session` フィールドを付与

   - **パターンB: 新規追加 (games[] / pending_games[] 両方未登録)**:
     - **判定条件**: 既存 records/{sport}/*.json に該当エントリが存在せず、完全新規追加。事後構築 (retrofit) を含む
     - **処理ルール 4項目**:
       1. 全フィールド (`predicted_winner` / `prediction_confidence` / `prediction_basis` / `odds` / 4本柱フィールド / `miss_analysis` 等) を新規構築
       2. 事後構築の場合は `prediction_basis_construction_note` フィールドを必須付与 (「実際の Session_X 予測ではない」と明示)
       3. **`prediction_hit_updated_at` フィールドを `step05_scanned_at` と同一 ISO 8601 UTC タイムスタンプで同時付与** (議題2 案γ 採用)
       4. `screened_session` を `_{session_id}_{task_id}_{step_id}_retrofit` 形式に統一 (例: `_64_a3_sub3_step4_retrofit` / case-by-case 対応も許容 / 既存形式 `_64_a3_sub3_session4` も許容)

   - **パターンC: 第3 のパターン (screening_log 痕跡あり + games[] 未登録)**:
     - **判定条件**: 既存 records/{sport}/*.json の `screening_log` 配列内に該当試合の痕跡が存在し、かつ games[] / pending_games[] 両方に登録未済
     - **処理ルール 6項目**:
       1. screening_log 痕跡から **`prediction_confidence` / `odds` / `tier` / `quadrant` の4要素を最低限復元可能であること**を確認 (復元不可ならパターンB として扱う / 議題2 論点4 確定方針)
       ...
       6. **`prediction_hit_updated_at` フィールドを `step05_scanned_at` と同一 ISO 8601 UTC タイムスタンプで同時付与** (議題2 案γ 採用)

   - **パターン共通: `prediction_hit_updated_at` フィールド規定 (議題2 案γ)**:
     - ISO 8601 UTC タイムスタンプ形式 (例: `"2026-04-28T12:18:01Z"`)
     - `step05_scanned_at` と **同一値で同時付与必須** (二重管理回避 / 値の派生関係: `step05_scanned_at` を一次値として `prediction_hit_updated_at` をミラー)
     - health_check 項目12/13 の検査対象判定 (第1優先キー) として機能 (本フィールド未付与だと記録が `last_updated_at` → `date` の順にフォールバックし、`date` (試合日) が古ければ承認日判定で skip される構造的問題を解消)
     - **遡及付与**: 議題2 step3 承認日 (= 2026-04-29) 以降の新規エントリ + サブタスク3 9件 (パターンA 4 + パターンB 4 + パターンC 1) に遡及付与
     - **記述例**:
       ```json
       {
         "match": "...",
         ...
         "step05_scanned_at": "2026-04-28T14:30:00Z",
         "prediction_hit_updated_at": "2026-04-28T14:30:00Z",
         "miss_class": "C",
         "miss_analysis": "..."
       }
       ```
   ...
   - **健全性検証 (機械検証)**:
     ...
     | health_check 項目 | 検証内容 |
     |---|---|
     | 12 (`miss_analysis_tag_compliance`) | `prediction_hit_updated_at` を第1優先キーとして検査対象判定 (承認日以降のエントリのみ走査) |
     | 13 (`step05_scan_compliance`) | 同上 |
     | 15 (`step05_prediction_hit_sync_compliance`) (新規) | `step05_scanned_at` 付与済 + `prediction_hit_updated_at` 未付与の検出 (完全未付与=ALERT / 部分未付与=WARN / 区分3 (`skip_record`) は走査対象外) |

   - **議題3 連携 (議題2 論点6 確定方針 / 議題3 step3 で実装済 / Session_64 2026-04-29)**: 本ステップ8 で確立されたパターンA/B/C は、**議題3 (柱D サブセクション9 テーブル + handoff 作成 SOP)** で参照される。柱D サブセクション9 テーブルに「処理パターン分類」列が追加され、9件全件 (パターンA 4 + パターンB 4 + パターンC 1) に分類値が遡及付与済。柱D サブセクション9 テーブル冒頭に本ステップ8 への参照リンクが明記済 (相互参照リンク完成)。柱D サブセクション 9-2 (handoff 作成 SOP) で本ステップ8 のパターンA/B/C 分類が参照される。
...
- **(Session_64 2026-04-29 議題2 追加) 項目12/13 第1優先キー = `prediction_hit_updated_at`**: health_check.py の項目12 (`miss_analysis_tag_compliance`) / 項目13 (`step05_scan_compliance`) 検査ロジックは、検査対象判定の第1優先キーに `prediction_hit_updated_at` を採用する (第2優先 = `last_updated_at` / 第3優先 = `date`)。本キー未付与だと `date` (試合日) が古いエントリは承認日判定で skip される構造的問題があり、サブタスク3 9件で 100% skip 率が発生していた。議題2 step3 で 9件への遡及付与 + サブセクション 2 ステップ8 規定により本問題を解消。
...
> **(Session_64 2026-04-29 議題2 追加)** 境界曖昧リーグの結果反映時の処理パターン (新規追加 / 既存遡及更新 / screening_log 痕跡再構築) は、**柱B サブセクション 2 ステップ8 (結果反映の処理パターン分類)** のパターンA/B/C 規定を参照すること。
...
>
> **(Session_64 2026-04-29 議題3 step3 追加)** 「処理パターン分類」列の値は **議題2 ステップ8 (柱B サブセクション 2 ステップ8) パターンA/B/C 規定** に従う (両方向リンク参照: 柱B ステップ8 → 本テーブル / 本テーブル冒頭 → 柱B ステップ8 / Session_64 議題3 step3 で相互参照リンク完成)。本テーブル運用時は併せて **サブセクション 9-2 (handoff 作成 SOP)** を必ず参照すること (handoff/CLAUDE.md への「柱D 9 テーブル相当」記述時の必須手順を規定)。

| # | 試合 | 推奨区分 | match_status | 処理パターン | 根拠 |
|---|---|---|---|---|---|
| 1 | Bristol 52-19 Newcastle Premiership | 区分1 (full_record) | completed | パターンA | scope内大会 / R14 / Q3_output_a 高信頼予測 HIT (BRI 1.04 fav 33点差大勝 / Bristol 52-19 Newcastle / records line 158 既存登録 / パターンA 該当) [FETCH:https://www.sofascore.com/rugby/match/newcastle-falcons-bristol-bears/QIbsRIb] |
| 2 | Northampton 41-38 Bath Premiership | 区分1 (full_record) | completed | パターンA | scope内 / R14 / Q4_upset_watch 予測 HIT (NSA 1.14 fav 3点差薄勝ち / Northampton 41-38 Bath / ユニオン系 7点差以内 = `confidence_drift: "high"` 付与済 (records L372 / Session_64 サブタスク3 セッション2 同時付与) / records line 322 既存登録 / パターンA 該当) [FETCH:https://www.skysports.com/rugby-union/northampton-vs-bath/111292] |
| 3 | Swiatek retire vs Li Madrid R3 | 区分2 (reference_only) | retired | パターンB | Swiatek (#1 シード) retire は UPSET 性質あり |
| 4 | Bondar d. Svitolina (#7 seed) Madrid R2 | 区分1 (full_record) | completed | パターンA | scope内 (WTA Masters 相当) / R2 / 試合成立 / #7 seed 敗退 (UPSET) |
| 5 | PHI G4 vs PIT (NHL Playoffs) | 区分1 (full_record) | completed | パターンC | scope内 / Playoffs / 試合成立 |
| 6 | TOR 93-89 CLE G4 (NBA Playoffs) | 区分1 (full_record) | completed | パターンB | scope内 / Playoffs / 市場fav CLE 敗戦 (UPSET) |
| 7 | HOU 115-96 LAL G4 (NBA Playoffs) | 区分1 (full_record) | completed | パターンC | scope内 / Playoffs G4 / 市場fav HOU 1.53 が 19点差大勝 HIT (HOU 1.53 fav [1.50 閾値超過 = 9-2 SOP §2 (b) scope外UPSET 候補除外対象の典型例] / Houston 115-96 Los Angeles / NBA 1桁差超過 = confidence_drift 不要 / records line 1614 事後構築 / パターンC 該当 (G3 screening_log L1953 → G4 prediction フィールド事後構築 / 議題4 step1 事前確認で訂正)) [FETCH:https://www.espn.com/nba/game/_/gameId/401869405] |
| 8 | Baptiste d. Paolini Madrid R3 | 区分1 (full_record) | completed | パターンA | scope内 (WTA Masters) / R3 / Paolini fav 敗戦 (UPSET) |
| 9 | Pliskova(PR) d. Mertens(#19) Madrid R3 | 区分1 (full_record) | completed | パターンB | scope内 / R3 / Mertens (#19) 敗戦 (UPSET) |

**処理パターン分類の根拠 (議題2 ステップ8 / Session_64 議題3 step3)**:
- **パターンA (既存遡及更新 / 4件)**: #1 Bristol-Newcastle (records line 158 既存登録) / #2 Northampton-Bath (records line 322 既存登録 + date 訂正) / #4 Bondar-Svitolina (records line 1535 既存登録 / screened_session=_50) / #8 Baptiste-Paolini (records line 2984 既存登録 / screened_session=_58)
- **パターンB (新規追加 / 3件)**: #3 Swiatek retire (upset_patterns A044 のみ登録) / #6 TOR-CLE G4 (新規追加) / #9 Pliskova-Mertens (records line 3027 新規追加 + retrofit)
- **パターンC (第3 のパターン / screening_log 痕跡再構築 / 2件)**: #5 PHI G4 vs PIT (records line 1985 / screening_log _58 痕跡から prediction_confidence/odds/tier/quadrant 4要素復元) / #7 HOU-LAL G4 (records line 1614 / Session_58 screening_log L1953 G3 言及から G4 prediction フィールド事後構築 / 議題4 step1 事前確認で訂正)

**遡及登録時の処理**: 4本柱完了後の別タスクにて、上記推奨区分に基づき records/{sport}/*.json への登録 (区分1) または upset_patterns.json への参考登録 (区分2) を実施する。Session_61 由来 12件 (Modified + Untracked) との統合実施を予定。**処理パターンごとの記録運用ルールは議題2 ステップ8 (柱B サブセクション 2 ステップ8) のパターンA 6項目処理ルール / パターンB 4項目処理ルール / パターンC 6項目処理ルールに従う**。

**訂正履歴**:
- 2026-04-28 A-3 サブタスク3 セッション1 ステップ5 で #9 試合表記を `Pliskova(Q)` → `Pliskova(PR)` に訂正 (WebFetch 確認結果反映 / Pliskova はキャリア中の Protected Ranking 利用エントリで Qualifier ではない)。
- 2026-04-29 Session_64 議題3 step3 で「処理パターン分類」列を新設し 9件全件に分類値を遡及付与 (パターンA 4 / パターンB 4 / パターンC 1 / 議題2 ステップ8 規定参照)。
- 2026-04-30 Session_64 議題4 step3 で #1 Bristol-Newcastle / #2 Northampton-Bath / #7 HOU-LAL G4 計3件の「根拠」列を訂正完了 (議題3 step4 運用テスト初実証 / 9-2 SOP (a)-(d) フロー実証完了)。
  - **#1**: 「市場fav 敗戦 (UPSET)」→ Q3_output_a 高信頼予測 HIT (BRI 1.04 fav 33点差大勝)。screening_log L447 (Session_53 Q3_output_a 1件として処理) → 結果反映 L456-465 (Session_64 サブタスク3 セッション2) 完全追跡可。
  - **#2**: 「scope内 / 既 records 登録 (date 訂正のみ必要)」→ Q4_upset_watch 予測 HIT + `confidence_drift: "high"` 付与済 (records L372 / Session_64 サブタスク3 セッション2 同時付与)。**date 訂正 (2026-04-26 → 2026-04-25)** は本訂正履歴エントリに統合 (skysports.com 一次ソース確認 = Saturday 25 April 2026 fixture / records L373 date_correction_note 既存記録)。
  - **#7**: 「市場fav LAL 敗戦 (UPSET)」→ 市場fav HOU が 19点差大勝 HIT (favorite が LAL ではなく HOU の二重誤判定訂正)。9-2 SOP §2 (b) 不適合 (HOU 1.53 が 1.50 閾値超過) の典型例として訂正。さらに議題3 step3 で「パターンB」遡及付与済だったが、議題4 step1 事前確認で screening_log L1953 (Session_58 G3 言及のみ / G4 直接スクリーニング記録なし) + records L1626 (`prediction_basis_construction_note`: G3 screening_log → G4 prediction フィールド事後構築明記) を再精査した結果、**パターンC (screening_log 痕跡再構築) が実態と整合することを確認 → パターンC に訂正**。本テーブル上の処理パターン件数は パターンA 4 / **パターンB 3** (-1) / **パターンC 2** (+1) に再集計。
  - **9-2 SOP §1 既存検索3経路 実証結果**: #1/#2 = records + screening_log + 過去 handoff 3経路すべて該当あり / #7 = records + screening_log + 過去 handoff 3経路すべて該当あり (G4 直接スクリーニング記録は Session_58 になく G3 言及から事後構築 = パターンC 該当根拠)
  - **9-2 SOP §2 (a)-(d) フロー実証**: 3件すべて (a) WebFetch 1件以上成功 (#1 sofascore / #2 skysports / #7 espn) + (b) market_fav オッズ閾値判定 (#1 BRI 1.04 適合 / #2 NSA 1.14 適合 / #7 HOU 1.53 不適合 = 除外対象典型例) + (c) records 整合性確認済 (3件すべて prediction_hit=true HIT) + (d) 不整合時の記述保留 → 訂正実施。

### 9-2. handoff 作成 SOP (Session_64 2026-04-29 議題3 step3 制定)

> **背景**: Session_61 由来 9件 振り分け案 (本サブセクション 9 テーブル) で #1 Bristol-Newcastle / #2 Northampton-Bath / #7 HOU-LAL G4 計3件の **誤記述 (33% 誤記述率)** が発生した。原因は (1) ad-hoc 判断による「市場fav 敗戦 (UPSET)」断定 (WebFetch 検証なし) / (2) 既存 records エントリ検索フェーズ不在 / (3) handoff 作成時の SOP 不在による Claude 判断揺れ、の3欠陥 (副次論点B)。本 SOP は handoff/CLAUDE.md への「柱D 9 テーブル相当」記述時の必須手順を規定し、誤記述構造を再発防止する。

#### 1. 既存検索フェーズ (必須3経路)

handoff 作成時に scope外UPSET 候補を扱う場合、必ず以下3経路を全件確認する:

| 経路 | 確認対象 | 用途 |
|---|---|---|
| (a) | `records/{sport}/*.json` の `games[]` / `predictions[]` / `pending_games[]` 配列 | 既存エントリ存在確認 → パターンA (既存遡及更新) 該当判定 |
| (b) | `screening_log` 配列 | screening_log 痕跡確認 → パターンC (第3 のパターン) 該当判定 |
| (c) | 既存 `handoff/*.md` / `CLAUDE.md` 内記述 | テーブル / 関連記述の重複・矛盾検出 |

3経路で既存エントリを検出 → **議題2 ステップ8 (柱B サブセクション 2 ステップ8) のパターンA/B/C 分類** を判定する:
- (a) で検出 + games[] エントリあり / 結果未反映 → **パターンA (既存遡及更新)**
- (a)/(b) いずれにも検出されない → **パターンB (新規追加)**
- (b) で検出 + games[] 未登録 + screening_log に prediction_confidence/odds/tier/quadrant の4要素復元可能 → **パターンC (第3 のパターン / screening_log 痕跡再構築)**
- (b) で検出だが 4要素復元不可 → パターンB として扱う (議題2 論点4 確定方針)

#### 2. scope外UPSET 性質事前確認 SOP (柱C Class C 規定遵守 / 統合形式)

handoff/CLAUDE.md に「市場fav 敗戦 (UPSET)」「scope外UPSET (市場fav 敗戦)」等の事実主張を記述する **前** に、必ず以下4ステップ (a)-(d) を実施する:

- **(a) WebFetch 1件以上で試合結果一次ソース確認** (柱C 4-2 Class C 規定遵守 / WebSearch スニペットのみは不可 / fetcher 経由構造化スタッツは `[FETCHER:src]` で 1件相当カウント可 / 推奨ソースは柱C 4-2 「claude_sport で参照する一次ソースサイト推奨リスト」参照)
- **(b) market_fav オッズ ≤1.50 (implied prob ≥66.7%) を一次ソース or fetcher 経由で確認** (オッズ ≤1.50 を満たさない場合は「市場fav」と称する閾値に届かないため、scope外UPSET 候補から除外する)
- **(c) records 上の `predicted_winner` / `prediction_hit` / `quadrant` / `market_fav` (or `favorite`) フィールドと整合性確認** (records 既登録試合の場合 / 未登録試合は (a)(b) のみで判定)
- **(d) 不整合時は記述保留** (records 実態に従う / handoff/CLAUDE.md への scope外UPSET 性質記述を断念し、records 実態 = HIT / MISS / SKIP のいずれかに従う)

**WebFetch 失敗時** (3回試行 NG): 柱C 4-2 規定の `[FETCH_FAILED:URL1,URL2,URL3]` タグ + `investigation_status: "investigation_incomplete"` を付与し、scope外UPSET 性質記述を **保留**。次セッション再試行を `monitoring/pending_actions.md` に登録する。

#### 3. テンプレート (handoff 内「柱D 9 テーブル相当」記述用 / 6列構造)

handoff 内で柱D 9 テーブル相当の振り分け案を記述する際は、以下6列構造のテンプレートに従う:

```
| # | 試合 (スコア + 対戦 + 大会) | 推奨区分 (区分1/2/3) | match_status (5値enum) | 処理パターン (パターンA/B/C / 議題2 ステップ8 参照) | 根拠 (records 実態に基づく自然言語記述 / 5種タグ運用 / 柱C 4-3 タグ義務) |
```
...
| 列 | 値の出所 |
|---|---|
| **試合** | scope外UPSET スキャン結果 (柱B サブセクション 2 ステップ2 で検出) + WebFetch でスコア確認 |
| **推奨区分** | 柱D サブセクション1 判定条件 (3区分定義) |
| **match_status** | 柱D サブセクション2 enum (5値: completed / retired / walkover / cancelled / postponed) |
| **処理パターン** | §1 既存検索フェーズ 3経路の結果 → 議題2 ステップ8 パターンA/B/C 分類 |
| **根拠** | (a)-(d) 4ステップ実施後の records 実態に基づく記述 (5種タグ義務適用) |

#### 4. Claude 判断揺れ防止チェックリスト (CHECK-2 連携)

handoff/CLAUDE.md への scope外UPSET 性質記述を行う前に、以下チェックリストを CHECK-2 自己点検 (`monitoring/check_protocol.md` CHECK-2) と連携して目視確認する:

- [ ] 既存検索フェーズ 3経路 (a)/(b)/(c) を **全件** 実施したか
- [ ] 「scope外UPSET (市場fav 敗戦)」「市場fav 敗戦 (UPSET)」等の記述前に **WebFetch 1件以上成功** したか (柱C 4-2 Class C 規定遵守)
- [ ] WebFetch 失敗時は `[FETCH_FAILED:URL1,URL2,URL3]` + `investigation_status: investigation_incomplete` 付与か
- [ ] records 上の `predicted_winner` / `prediction_hit` / `quadrant` / `market_fav` と整合性確認したか
- [ ] 不整合時は記述を保留し records 実態に従ったか
- [ ] 処理パターン分類 (パターンA/B/C) は議題2 ステップ8 規定通りか
- [ ] 6列構造テンプレート (§3) に従っているか
- [ ] 柱C 4-3 5種タグ義務 (`[FETCH:URL]` / `[FETCHER:src]` / `[SEARCH]` / `[MEMORY]` / `[INFER]` / `[FETCH_FAILED:...]`) を主張ごとに付与したか

#### 5. SOP 違反時の事後対応

本 SOP 違反による誤記述が事後発覚した場合 (= Session_64 議題4 で実施予定の #1/#2/#7 訂正の前例):

1. 誤記述箇所を特定 (handoff / CLAUDE.md 柱D 9 テーブル / 関連記録)
2. records 実態を確認 (既存検索フェーズ 3経路を再実施)
3. (a)-(d) 4ステップで scope外UPSET 性質を再判定
4. 訂正実施 + 訂正履歴を「**訂正履歴**」セクションに必須記載 (柱D 9 テーブル冒頭の訂正履歴セクション形式に従う)
5. 誤記述率が 30% 超の場合は本 SOP の構造的欠陥を再検証 (議題化候補)

#### 6. 既存柱との整合性

- **柱A (rule 改訂承認制)**: 本 SOP は柱D サブセクション9 テーブル運用に限定され、rules_*.json / rule_pipeline.json への直接影響なし
- **柱B サブセクション 2 ステップ8 (パターンA/B/C 明文化)**: §1 既存検索フェーズで本ステップ8 のパターンA/B/C 分類を必須参照 (両方向リンク)
- **柱C 4-2 (fetch 件数規定)**: §2 (a) で Class C 規定 (1件以上) を遵守
- **柱C 4-3 (情報源タグ義務化)**: §4 チェックリストで5種タグ付与を CHECK-2 連携で確認
- **柱D サブセクション1 (3区分定義)**: §3 テンプレートの「推奨区分」列で参照
- **柱D サブセクション2 (match_status enum)**: §3 テンプレートの「match_status」列で参照
