...
⑦ MISSがあれば miss_analysis に要因分類(L1/L2/L3/L4/External)を記録
   ※ GEN009 v2 (構造v3): 全確定試合に verification 必須 (class+depth+facts+analysis+rules_implication+source_tier)
     - class: hit_normal / hit_unexpected / miss / upset_occurred / market_divergence から1〜複数
     - depth: hit_normal のみ→light(facts 20字以上) / それ以外含む→deep(facts 40字以上 + analysis 全項目必須)
     - facts: スニペット由来事実のみ(推論禁止、出典明記必須)
     - analysis: 推論OK、confidence_level(high/medium/low/speculative)で確度自己申告(deepのみ必須)
     - rules_implication: ルール候補があれば記入(任意、light/deep共通)
     - 境界事例(Q1〜Q4 / skip / VOID/WALKOVER 等)は core/rules_general.json GEN009 v2 boundary_rules で整理済
